...
# name of the stylesheet is javadoc2assertions-with-ctor-exceptions.xsl).  The output
...
USAGE="usage: ${0} technology id name version source_dir output_dir package_name_1 [package_name_2 ...] assertion number prefix[example: 'JSF:JAVADOC:']"
...
#####
cd ../xml-doclet
CMD="ant -Dinclude.ctor.exceptions=true -Dinclude.comment.exceptions=true -Dprocess.inherited.members="true" -Dsrc-path=$SOURCE_DIR -Doutput-file=${OUTPUT_DIR}/${OUTPUT_FILE} -Dpackages-for-docs=${PACKAGE_ARG}"
echo "\n\n$CMD\n\n"
$CMD
...
#	Note:
# 			The style sheet we use here will mark all Abstract member's testable attribute as false.
#####
cd ../xsl-transformer/scripts
XML_ASSERT_NO_NUMS_FILE="assertions-no-nums.xml"
XSL_FILE="../../../docs/xsl/assertions/javadoc2assertions-inherited-with-ctor-exceptions.xsl"
...
# javadoc2assertions-with-ctor-exceptions.xsl stylesheet above
#####
...
CMD="ant -Dprefix=${ASSERT_NUM_PREFIX} -Dinput_file=${INPUT_FILE} -Doutput_file=${OUTPUT_FILE}"
echo "\n\n$CMD\n\n"
$CMD
